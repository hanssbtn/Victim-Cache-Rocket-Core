...
// 3. Block Stride: Access index 0, 64, 128... 
// Skips spatial locality within a block, testing tag logic efficiency.
void test_block_stride() {
    printf("Starting Test 3: Block Stride\n");
    for (int i = 0; i < (ARRAY_SIZE/8); i += (BLOCK_SIZE/8)) {
        buffer[i] = i + 1;
    }
    printf("Ending Test 3: Block Stride\n");
}
