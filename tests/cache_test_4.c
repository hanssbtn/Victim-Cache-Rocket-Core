...
// 4. Matrix Multiply (Small): Classic workload with row vs column access patterns.
#define N 32
volatile uint64_t A[N][N], B[N][N], C[N][N];
void test_matmul() {
    printf("Starting Test 4: MatMul %dx%d\n", N, N);
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            for (int k = 0; k < N; k++) {
                C[i][j] += A[i][k] * B[k][j];
    ...
    printf("Ending Test 4: MatMul %dx%d\n", N, N);
}
