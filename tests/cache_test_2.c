...
// 2. Thrashing (Ping-Pong): Access addresses that map to the SAME set index
// but different tags. This forces evictions.
// Access A, B, C, D, E (E evicts A). Then Read A (Hit in VC?).
void test_set_thrashing() {
    printf("Starting Test 2: Set Thrashing\n");
    int stride_elems = SET_SIZE / 8; 
    
    // We purposefully overflow the ways (NUM_WAYS + 1)
    // 5 accesses to the same set index
    for (int k = 0; k < 100; k++) { // Repeat to accumulate stats
        for (int way = 0; way <= NUM_WAYS; way++) {
            buffer[way * stride_elems] = 1; 
        }
        // Now read back the first one (which should have been evicted to VC)
        volatile uint64_t temp = buffer[0];
    }
    printf("Ending Test 2: Set Thrashing\n");
}
