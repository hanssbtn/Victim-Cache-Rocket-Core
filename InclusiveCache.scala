    ...
    extends LazyModule
{
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    require (ctrls.isEmpty || !node.edges.in.isEmpty, s"${ctrls.isEmpty} || ${!node.edges.in.isEmpty}") 
    ...
    node.edges.in.foreach { e => require(e.client.clients.map(_.sourceId).sortBy(_.start) == clientIds.get, "e.client.clients.map(_.sourceId).sortBy(_.start) == clientIds.get") }
